...
function makeRotateEffect() {
  try {
    ...
    return make(ROTATE_LAYER_SKSL);
  } catch {
    return null;
  }
}

let rotateEffectCache: ReturnType<typeof makeRotateEffect> | undefined;
function getRotateEffect() {
  if (rotateEffectCache === undefined) {
    rotateEffectCache = makeRotateEffect();
  }
  return rotateEffectCache;
}
...
export function RotateBeam({
  ...
  const rotateEffect = getRotateEffect();
  ...
  // Number for worklets — avoid free `isDark` lookup on the UI thread.
  const bgIsBlack = isDark ? 0 : 1;
  ...
  }, [baseLayer, s, bgIsBlack, strength, monoMul, colors.strokeOpacity]);
  ...
  }, [baseLayer, s, bgIsBlack, strength, monoMul, colors.bloomOpacity, bloomMatrix]);

  if (!rotateEffect || width <= 0 || height <= 0) return null;
