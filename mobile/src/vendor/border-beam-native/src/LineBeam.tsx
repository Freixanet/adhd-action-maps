...
function makeBlobEffect() {
  try {
    ...
    return make(BLOB_LAYER_SKSL);
  } catch {
    return null;
  }
}

let blobEffectCache: ReturnType<typeof makeBlobEffect> | undefined;
export function getBlobEffect() {
  if (blobEffectCache === undefined) {
    blobEffectCache = makeBlobEffect();
  }
  return blobEffectCache;
}
...
export function LineBeam({
  ...
  const blobEffect = getBlobEffect();
  const spec = beamSpec;
