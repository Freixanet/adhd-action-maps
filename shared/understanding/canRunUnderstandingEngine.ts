/**
 * S04 — decide whether Understanding Engine can run on this request.
 * Based on real extracted canonical text, not intent alone.
 */

import type { TransformRequest } from '../contracts';
import type { IngestResult } from '../types/chunk';

export type UnderstandingRouteDecision =
  | { run: true; reason: 'extracted_text' | 'pasted_text' | 'web_or_document_text' }
  | {
      run: false;
      reason:
        | 'not_understand_intent'
        | 'ask_lane'
...
function looksLikeBareHttpUrl(text: string): boolean {
  ...
  // Single-token URL (no substantial body after)
...
/**
 * Shared gate for JSON and NDJSON handlers.
 * `ingestKind` comes from resolveTransformIngest / prepareTransformIngest.
 */
export function canRunUnderstandingEngine(args: {
  ...
  if (intent !== 'understand') {
    return { run: false, reason: 'not_understand_intent' };
  }
  ...
  if (type === 'youtube') {
    // Transcript-first legacy until YouTube is wired as textual ingest for S04.
  ...
  // Guard: never treat a bare URL as source content for the engine.
  ...
  if (args.ingestKind === 'source' && args.ingest?.chunks?.length) {
    const isPaste =
      args.body.type === 'text' &&
      !looksLikeBareHttpUrl(text.slice(0, 200));
    return {
      run: true,
